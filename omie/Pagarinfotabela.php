...
foreach ($ret->conta_pagar_cadastro as $dados) {
            ...
            if( strtotime($dataamericana) >= strtotime($dataI) and strtotime($dataamericana) <= strtotime($dataF)){
                ...
                $objPHPExcel->getActiveSheet()->SetCellValue('H'.$contador, "INFORMÁTICA");
                ...
                array_push($valor, number_format($dados->valor_documento,2, '.', ''));
                  ...
                  echo "CLIENTE/FORNECEDOR: "."<button type='button' id='codigocliente' class='Recebertabelainfocodcliente' data-toggle='modal' data-target='#exampleModal'>".$dados->codigo_cliente_fornecedor."</button><BR>";
...
$objPHPExcel->getActiveSheet()->SetTitle("PagarInformatica");
