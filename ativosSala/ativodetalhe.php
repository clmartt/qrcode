...
<html lang="pt-br">
 ...
 foreach ($result as $resultado) {
  ...
  echo '<p class="card-text">PREDIO : '.$resultado["PREDIO"].' - '.'ANDAR : '.$resultado["ANDAR"].'  <br> SETOR : '.$resultado["SETOR"].' - SALA : '.utf8_encode($resultado["SALA"]).'</p>';
   echo '<p class="card-text">'.$resultado["TIPO_DE_EQUIPAMENTO"].' - CARACTERISTICA : '.$resultado["CARACTERISTICA"].'<BR>HORAS LAMP : '.$resultado["HORAS_LAMP"].'<BR>MARCA : '.utf8_encode($resultado["MARCA"]).' <br> MODELO : '.$resultado["MODELO"].'<BR>N_SERIE : '.$resultado["N_SERIE"].'</p>';
  echo '</div>';
  echo '</div>';
