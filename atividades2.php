...
$selecao = "SELECT * FROM AGENDAMENTO WHERE RECURSO = '$usuario' and DATA_AGEN = '$dataHoje' ORDER BY HORA_INICIO"; // seleciona os agendamentos pelo usuario e data de hoje ordenando por hora de inicio
...
<div class="container mt-3">
  Atividades Agendadas - <?php echo date('d/m/Y'). " - ". $usuario?> 
  <hr>
  <p></p>
...
<?php 
  ...
  foreach ($resultado as $res) {
    ...
    if($situacao=='ABERTO'){
      ...
      echo utf8_encode($res['TITULO']);
      echo '</div>';
      echo '<div class="card-body">';
      echo '<h5 class="card-title">'.utf8_encode($res['TITULO']).'</h5>';
      echo '<p class="card-text"> Responsável : '.$res['RESPONSAVEL'].'</p>';
      echo '<p class="card-text"> Tipo : '.$res['TIPO'].'</p>';
    ...
    if($situacao=='ANDAMENTO'){
      ...
      echo '<p class="card-text"> Atividade : '.$res['ATIVIDADE'].'</p>';
      echo '<p class="card-text"> Tipo : '.$res['TIPO'].'</p>';
      echo '<p class="card-text">'.$res['OBSERVACAO'].'</p>';
