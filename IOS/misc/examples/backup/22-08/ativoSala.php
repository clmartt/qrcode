...
$temppredio = $res2['PREDIO'];
$tempsala = $res2['SALA'];
$tempandar = $res2['ANDAR'];
$select3 = "SELECT * FROM QRCODETABLE WHERE PREDIO = '$temppredio' AND SALA = '$tempsala' AND ANDAR = '$tempandar'";
$result3 = $pdo->query($select3);

              foreach($result3 as $res3){
               
                echo "    - ".trim("<a href=ativodetalhe.php?qrcode=".$res3['QRCODE'].">").$res3['QRCODE']."</a>".' - '.utf8_encode($res3['TIPO_DE_EQUIPAMENTO']).' - '.utf8_encode($res3['CARACTERISTICA']).'<br> ';
